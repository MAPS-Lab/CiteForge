...
from tests.test_data import API_SPECIFIC_PAPERS, KNOWN_PAPERS
...
def test_serpapi_scholar_citation(api_keys):
    ...
    try:
        ...
        fields = scholar.fetch_scholar_citation_via_serpapi(
            api_keys['serpapi'],
            author_id,
            citation_id
        )
...
def test_multiple_candidate_empty_inputs():
    ...
    # Test max_results=0
    candidates = search_apis.crossref_search_multiple("Attention Is All You Need", "Ashish Vaswani", max_results=0)
...
def test_bibtex_building_from_api_responses():
    ...
    if cr_item:
        bibtex = search_apis.build_bibtex_from_crossref(cr_item, paper['first_author'])
