...
# Integration tests that hit external APIs require a SerpAPI key.
...
def test_full_enrichment_pipeline(api_keys):
    ...
    cite_link = scholar.search_scholar_for_cite_link(
        api_keys['serpapi'],
        paper['title'],
        paper['first_author']
    )
    ...
    if api_keys.get('semantic'):
        try:
            s2_paper = search_apis.s2_search_paper(
                paper['title'],
                paper['first_author'],
                api_keys['semantic']
            )
            if s2_paper:
                s2_bib = search_apis.build_bibtex_from_s2(s2_paper, paper['first_author'])
    ...
    try:
        cr_item = search_apis.crossref_search(paper['title'], paper['first_author'])
        if cr_item:
            cr_bib = search_apis.build_bibtex_from_crossref(cr_item, paper['first_author'])
    ...
    if paper['arxiv_id']:
        try:
            arxiv_entries = search_apis.arxiv_search(
                paper['title'],
                paper['first_author'],
                paper['year']
            )
            if arxiv_entries:
                arxiv_bib = search_apis.build_bibtex_from_arxiv(
                    arxiv_entries[0],
                    paper['first_author']
                )
    ...
    if paper['doi']:
        try:
            ...
            if csl:
                csl_bib = search_apis.bibtex_from_csl(csl, paper['first_author'])
